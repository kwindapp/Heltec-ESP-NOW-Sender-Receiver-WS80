...
const char* mqtt_server = "1xxxxxxxxxx";  // Replace with your MQTT server address
...
void displayData() {
  // Show "New Data Received" message briefly
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_ncenB14_tr);
  u8g2.drawStr(10, 20,"NEW DATA");
  u8g2.setCursor(10, 55);
  u8g2.print(useMQTT ?"MQTT ON":"MQTT OFF");
  u8g2.sendBuffer();
  delay(500);  // Delay to keep message on screen

  // Now show actual sensor data
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_6x10_tf);

  String windDirStr = String(receivedData.windDir) + "  " + getCardinalDirection(receivedData.windDir);
  String windSpeedStr = String(convertWindSpeed(receivedData.windSpeed), 1) + (useKnots ? "  knt" : " m/s");
  String windGustStr = String(convertWindSpeed(receivedData.windGust), 1) + (useKnots ? "  knt" : " m/s");

  u8g2.drawStr(0, 10, ("Wind  :  " + windSpeedStr).c_str());
  u8g2.drawStr(0, 20, ("Gust  :  " + windGustStr).c_str());
  u8g2.drawStr(0, 30, ("Dir   :  " + windDirStr).c_str());
  u8g2.drawStr(0, 40, ("Temp  :  " + String(receivedData.temperature, 1) + " C").c_str());
  u8g2.drawStr(0, 50, ("Bat   :  " + String(receivedData.BatVoltage, 2) + " V").c_str());
  u8g2.drawStr(0, 60, ("Humi  :  " + String(receivedData.humidity, 1) + " %").c_str());
